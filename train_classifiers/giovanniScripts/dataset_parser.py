...
def parse_file(file_to_parse, aggregation=1, file_to_save=None, verbose=False):
    ...
    # Parsing PAN 2018 xml formatted documents:
    # <author lang="XX">
    #   <documents>
...
def parse_tweets_from_dir(input_dir, splitDic, output_dir=None, label=True, aggregation=100, verbosity_level=1) :
    '''
    ...
    try:
        xml_files = [f for f in listdir(input_dir+"/text") if (
            isfile(join(input_dir+"/text", f)) and f[-4:] == ".xml" )
                     and splitDic[f.strip(".xml")] == 0
                     ]
    except:
    ...
    for f in xml_files :
        ...
        try:
            author = parse_file(
                file_to_parse=input_dir +"/text/" + f,
...
def parse_tweets_from_dir_2(input_dir, list_authors, output_dir=None, label=True, aggregation=100, verbosity_level=1):
    '''
    Parses all the xml files directly in the input_dir (no recursion).
    Retrieves the attributes of the author stored in the truth file.
    If specified, the parsed files will be written into the output_dir
